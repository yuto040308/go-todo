...
│  └─ (protected)/            # Route Group。配下は layout で自動ガード
│     ├─ layout.tsx           #   クライアント側 AuthGuard
│     └─ todos/
│        ├─ page.tsx          #   一覧
│        ├─ new/page.tsx      #   新規作成
│        └─ [id]/edit/page.tsx#   編集
...
- `app/(protected)/layout.tsx`（Client Component）で token を判定し、無ければ `router.replace('/login')`
- 保護対象は `(protected)/` 配下に置くだけで守られる（Route Group はURLに出ない）
...
- `app/todos/page.tsx` を `app/(protected)/todos/` 配下へ移設し、モックを実データ（React Query）に置換
