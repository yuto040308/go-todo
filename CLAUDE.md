...
- `frontend/` — Next.js
  ...
  - `components/ui/` — shadcn/ui 生成物 (`npx shadcn add` で radix-nova スタイル生成)。**編集可** — 所有コードとして扱う生成物 (編集禁止の `types/api.ts`・`gen/api/` とは別カテゴリ)。逆に `components/ui/` 以外のコンポーネントは自作物、と区別する
  - `api/` — axios ラッパ層
...
- **Data API / RLS**: Supabase は public スキーマの全テーブルを Data API (PostgREST) で自動公開し、publishable key (公開鍵) で外部から全行読めてしまう (実際に 2026-07-09 に `users.password_hash` 露出が発覚)。バックエンドは Data API を使わないので、**public の全テーブルに RLS を有効化** (ポリシー無し = anon 全拒否) して裏口を塞ぐ。RLS は**テーブル単位**なので新規テーブル追加時は必ず有効化 (付け忘れ検知に Supabase の Security Advisor を活用)。バックエンドは BYPASSRLS を持つ `postgres` role 接続なので CRUD への影響は無い
...
- **本番マイグレーション** — 起動時に embed + golang-migrate で `migrate up` ([main.go](backend/main.go) の `runMigrations`)。デプロイのたびに新リビジョンが適用する。⚠️ DB 到達不可だと起動失敗 = 全停止しうる (2026-07-09 の Supabase pause 事例) ため、DB 可用性 (keep-alive) とセットで運用する
